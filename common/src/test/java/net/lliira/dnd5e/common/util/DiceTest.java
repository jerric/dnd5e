...
public class DiceTest {
  ...
  private static void assertInRange(int actual, int expectedFrom, int expectedTo) {
    Assert.assertTrue(
        String.format(
            "actual(%d) not in expected range [%d, %d]", actual, expectedFrom, expectedTo),
        actual >= expectedFrom && actual <= expectedTo);
  }

  private static void assertDice(
      Dice actual, int expectedSides, int expectedCount, int expectedModifier) {
  ...
  private static void assertField(String fieldName, int actual, int expected) {
    Assert.assertEquals(
        String.format("%s: %d != %d", fieldName, expected, actual), expected, actual);
  }
}
